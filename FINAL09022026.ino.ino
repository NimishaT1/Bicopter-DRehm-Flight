...
const float DIVIDER_RATIO = 4.0 ;   // Voltage sensor module (≈5:1)
const float CAL_FACTOR = 1.0 ;     // Calibrated by measurement
...
void setup() {
  ...
  delay(500);
  Serial2.begin(115200);   // UART to ESP8266 (TX = pin 8)
...
float readBatteryVoltage() {
  ...
  // Low-pass filter (flight-safe)
  filtered = 0.9 * filtered + 0.1 * voltage;
...
void loop() {
  ...
  if (armedFly) {
  servo3.write(m1_command_PWM);
  servo4.write(m2_command_PWM);
  } 
  else {
  servo3.write(0);   // motors OFF
  servo4.write(0);
  }
...
m2_command_scaled = constrain(m2_command_scaled, 0.0, 1.0);
  ...
  float pitch = pitch_PID;
  float yaw   = yaw_PID;

  float k = 0.3;   // keep small to avoid saturation

  // LEFT servo
  s1_command_scaled = 0.5 + k*pitch + k*yaw;

  // RIGHT servo
  s2_command_scaled = 0.5 + k*pitch - k*yaw;
...
void armedStatus() {
  ...
  // Motor arming based on throttle position only

  if (channel_3_pwm < 1050) {   // stick fully down
    armedFly = false;           // motors OFF
  } else {
    armedFly = true;            // motors ON
  }
}
...
void getDesState() {
  ...
  // THROTTLE -> left stick up/down (CH3)
  thro_des = (channel_3_pwm - 1000.0) / 500.0;

  // ROLL -> left stick left/right (CH4)
  roll_des = (channel_4_pwm - 1500.0) / 500.0;

  // PITCH -> right stick up/down (CH2)
  pitch_des = (channel_2_pwm - 1500.0) / 500.0;

  // YAW -> right stick left/right (CH1)
  yaw_des = (channel_1_pwm - 1500.0) / 500.0;
...
void controlANGLE2() {
  ...
  //Pitch
  error_pitch = pitch_des - roll_IMU;
...
void scaleCommands() {
  ...
  // ===== ESCs (using PWMServo angle 0–180) =====
  ...
  s2_command_PWM = 180 - s2_command_PWM;   // reverse right servo (keep)
...
void calibrateESCs() {
   ...
   */
   while (true) {
      ...
      loopRate(400); //Do not exceed 2000Hz, all filter parameters tuned to 2000Hz by default
   }
}
...
void sendTelemetryESP() {
  // Send at ~50 Hz (safe, non-blocking)
  if (micros() - serial_counter > 20000) {
    serial_counter = micros();

    Serial2.print(roll_IMU, 2);
    Serial2.print(",");
    Serial2.print(pitch_IMU, 2);
    Serial2.print(",");
    Serial2.print(yaw_IMU, 2);
    Serial2.print(",");
    Serial2.print(thro_des, 3);
    Serial2.print(",");
    Serial2.println(batteryVoltage, 2);
