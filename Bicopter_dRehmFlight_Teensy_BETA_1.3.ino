...
float GyroErrorX = -4.60;
float GyroErrorY= -0.71;
...
float Kp_roll_angle = 0.40;    //Roll P-gain - angle mode 
float Ki_roll_angle = 0.30;    //Roll I-gain - angle mode
float Kd_roll_angle = 0.05;   //Roll D-gain - angle mode (has no effect on controlANGLE2)
float B_loop_roll = 0.9;      //Roll damping term for controlANGLE2(), lower is more damping (must be between 0 to 1)
float Kp_pitch_angle = 0.40;   //Pitch P-gain - angle mode
float Ki_pitch_angle = 0.30;   //Pitch I-gain - angle mode
...
//  pinMode(m6Pin, OUTPUT);
  ...
  m1.attach(m1Pin, 1000, 2000);
  m2.attach(m2Pin, 1000, 2000);
  m3.attach(m3Pin, 1000, 2000);
  m4.attach(m4Pin, 1000, 2000);
  m5.attach(m5Pin, 1000, 2000);
  m6.attach(m6Pin, 1000, 2000);
  ...
  m1_command_PWM = 0; //Command OneShot125 ESC from 125 to 250us pulse length
  m2_command_PWM = 0;
  m3_command_PWM = 0;
  m4_command_PWM = 0;
  m5_command_PWM = 0;
...
void loop() {
  ...
  //Serial.println(fading_variable);
  //Get vehicle state
  ...
  m1.write(m1_command_PWM); //Writes PWM value to servo object
  m2.write(m2_command_PWM);
  m3.write(m3_command_PWM);
  m4.write(m4_command_PWM);
  m5.write(m5_command_PWM);
  m6.write(m6_command_PWM);
...
void controlMixer() {
   ...
   */
  // Bicopter mixing
  // Motors: m1 = left motor, m2 = right motor
  // We use differential thrust for roll, symmetric thrust for collective (throttle)
  // Yaw & pitch control are achieved primarily via two tilting servos (s1 and s2).
  // Tunable scales below (tweak in flight testing):
  float bicopter_servo_pitch_scale = 0.02; // maps pitch PID to servo tilt (centered at 0.5)
  float bicopter_servo_yaw_scale = 0.02;   // maps yaw PID to differential tilt

  // Motor mixing: differential roll
  ...
  // Servo mixing: s1 and s2 control motor tilt.
  // sX_command_scaled is 0..1 where 0.5 is neutral (90 degrees).
  // Apply pitch as symmetric opposite tilt, yaw as differential tilt.
  float servo_center = 0.5;
  s1_command_scaled = servo_center + (bicopter_servo_pitch_scale * pitch_PID) + (bicopter_servo_yaw_scale * yaw_PID);
  s2_command_scaled = servo_center + (bicopter_servo_pitch_scale * pitch_PID) + (bicopter_servo_yaw_scale * -yaw_PID);

  // Keep other servos neutral
...
//  m6_command_PWM = constrain(m6_command_PWM, 125, 250);
  ...
  m5_command_PWM = m5_command_scaled*180;
  m6_command_PWM = m6_command_scaled*180;
  ...
  m5_command_PWM = constrain(m5_command_PWM, 0, 180);
  m6_command_PWM = constrain(m6_command_PWM, 0, 180);
}
